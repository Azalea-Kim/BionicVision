...
sys.path.append(os.path.join(project_root, "src", "models", "segmentation", "mit_scene_parsing"))
...
colors = scipy.io.loadmat(local_dir + '\\src\models\segmentation\mit_scene_parsing\\data\\color150.mat')['colors']
names = {}
with open(local_dir + '\\src\models\segmentation\mit_scene_parsing\\data\\object150_info.csv') as f:
    reader = csv.reader(f)
    next(reader)
    for row in reader:
...
net_encoder = ModelBuilder.build_encoder(
    ...
    fc_dim=2048,
    weights=local_dir + '\\src\models\segmentation\mit_scene_parsing\\ckpt\\ade20k-resnet50dilated-ppm_deepsup\\encoder_epoch_20.pth')
net_decoder = ModelBuilder.build_decoder(
    arch='ppm_deepsup',
    fc_dim=2048,
    num_class=150,
    weights=local_dir + '\\src\models\segmentation\mit_scene_parsing\\ckpt\\ade20k-resnet50dilated-ppm_deepsup\\decoder_epoch_20.pth',
    use_softmax=True)
