...
def correct_interpolations(in_folder, out_folder, specific_json_list=[]):
    ...
    for infile in tqdm(sorted(glob.glob(in_folder + '/*_interpolations.json'))):
...
if __name__ == '__main__':
    ...
    add_mssing_sparse_annotations(args.output_dir,['P02_01']) #add some missing sparse masks into these 2 videos
    print('Doing the final checks')
    correct_interpolations(args.output_dir,args.output_dir,specific_json_list = ['P02_01']) #fix those videos after adding the missing frames
