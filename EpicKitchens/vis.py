...
def generate_masks_for_image(image_name, image_path, masks_info, output_directory,object_keys=None,is_overlay=False, images_root_directory= '.', input_resolution=(1920,1080), output_resolution=(1920,1080), save_numpy_masks=False):
    ...
    if (not np.all(img == 0)):
        ...
        # Save raw numpy array with object IDs
        if save_numpy_masks:
            numpy_dir = os.path.join(output_directory, 'numpy_masks')
            os.makedirs(numpy_dir, exist_ok=True)
            numpy_filename = image_name.replace('.png', '.npy')
            np.save(os.path.join(numpy_dir, numpy_filename), out_image)
...
def folder_of_jsons_to_masks(json_files_path,output_directory,is_overlay=False, rgb_frames= '.', output_resolution=(1920,1080),generate_video=True, save_numpy_masks=False):
    ...
    for json_file in tqdm(sorted(glob.glob(os.path.join(json_files_path ,'*.json')))):
        if 'interpolation' in os.path.basename(json_file):
            ...
            frame_rate = 3
        objects_keys = json_to_masks_new(json_file,output_directory,is_overlay=is_overlay, images_root_directory=rgb_frames,input_resolution=input_resolution, output_resolution=output_resolution, save_numpy_masks=save_numpy_masks)
        if generate_video:
...
def json_to_masks_new(filename,output_directory,is_overlay=False, images_root_directory= '.',input_resolution=(1920,1080), output_resolution=(1920,1080), save_numpy_masks=False):
    ...
    for datapoint in data:
        ...
        generate_masks_for_image(image_name, image_path, masks_info, full_path,object_keys=object_keys,is_overlay=is_overlay,images_root_directory=images_root_directory,input_resolution=input_resolution, output_resolution=output_resolution, save_numpy_masks=save_numpy_masks) #this is for unique id for each object throughout the video
    return object_keys
