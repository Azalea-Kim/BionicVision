...
def load_video_paths(args):
    root_path = args.input  # root folder containing image frames
    frame_names = sorted(os.listdir(root_path))  # Get all file names in the folder
    frame_paths = [os.path.join(root_path, name) for name in frame_names if name.lower().endswith(('jpg', 'jpeg', 'png', 'bmp'))]  # Filter image files
...
def run(args):
    ...
    for i in range(len(path_lists)):
        ...
        for j in range(len(output_list)):
            ...
            # Create a filename for each frame (e.g., frame_001.png, frame_002.png, etc.)
            frame_filename = os.path.join(args.output, f"frame_{j + 1:03d}.jpg")  # 003 padding for numbering

            # Save the colored frame as an image
            cv2.imwrite(frame_filename, frame_color)
...
if __name__ == "__main__":
    ...
    run(args)
#python demo.py --model large --resume ./weights/_ckpt.pt.tar --input D:\2021-han-scene-simplification-master\2021-han-scene-simplification-master\output_frames\kitchen20fps  --output D:\2021-han-scene-simplification-master\2021-han-scene-simplification-master\depth_output_npy\kitchen20fps_TCMono
